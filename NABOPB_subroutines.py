...
device = "cpu"
if device == "cuda":
    prec_type = torch.float
else:
    prec_type = torch.double


def random_number(d, dims, basis, number_points=1):
    """
    ...
    x_tilde = np.zeros((number_points, d - dims.size))
    ...
    if basis in {
        ...
        x_tilde = np.random.rand(number_points, d - dims.size)

    elif basis in {
        "Cheby_rand",
        "Cheby_mr1l",
        "Cheby_mr1l_2024",
        "Cheby_mr1l_subsampling",
        "Cheby_SRLasso",
        "Cheby_SRLasso+",
        "Cheby_OMP",
        "Cheby_OMP+",
        ...
        x_tilde = np.sin(np.pi * (np.random.rand(number_points, d - dims.size) - 1 / 2))
...
def construct_cubature_1d(K, basis, j, d, s):
    """
    ...
    if basis == "Fourier_rand":
        ...
        W = s
        Xi = np.sin(np.pi * (np.random.rand(1 * W) - 0.5))
...
def construct_cubature(K, basis, dims, d, s, pde_dims=None):
    """
    ...
    if basis == "Fourier_rand":
        ...
        Xi[0 : Ms[0], :] = (
            np.mod(np.arange(Ms[0]).reshape(-1, 1) * zs[0], Ms[0]) / Ms[0]
        )
        for j in range(1, len(Ms)):
            Xi[np.sum(Ms[:j]) - j + 1 : np.sum(Ms[: j + 1]) - j, :] = (
                np.mod(np.arange(1, Ms[j]).reshape(-1, 1) * zs[j], Ms[j]) / Ms[j]
            )
    elif basis in {"Fourier_SRLasso", "Fourier_OMP", "Fourier_COSAMP"}:
        W = s
        t = K.shape[1]
        Xi = np.random.rand(10 * W, t)
    elif basis in {"Fourier_SRLasso+", "Fourier_OMP+", "Fourier_COSAMP+"}:
        W = s
        t = K.shape[1]
        Xi = np.random.rand(1 * W, t)
    elif basis == "Cheby_rand":
        # Monte Carlo, W is number of nodes
        n = np.sum(2 ** (np.sum(K != 0, axis=1)))
        W = int(np.ceil(n * np.log(n)))
        ...
        Xi[: ((Ms[0] + 1) // 2), :] = np.cos(
            2
            * np.pi
            * np.mod(np.arange((Ms[0] - 1) // 2 + 1).reshape(-1, 1) * zs[0, :], Ms[0])
            / Ms[0]
        )
        zaehler = (Ms[0] + 1) // 2
        for j in range(1, len(Ms)):
            Xi[zaehler : zaehler + (Ms[j] - 1) // 2, :] = np.cos(
                2
                * np.pi
                * np.mod(
                    np.arange(1, (Ms[j] - 1) // 2 + 1).reshape(-1, 1) * zs[j, :], Ms[j]
                )
                / Ms[j]
            )
            zaehler += (Ms[j] - 1) // 2
    ...
    elif basis in {"Cheby_SRLasso", "Cheby_OMP", "Cheby_COSAMP"}:
        W = s
        t = K.shape[1]
        Xi = np.sin(np.pi * (np.random.rand(5 * W, t) - 0.5))
    elif basis in {"Cheby_SRLasso+", "Cheby_OMP+", "Cheby_COSAMP+"}:
        W = s
        t = K.shape[1]
        if max(dims) > pde_dims:
            Xi = np.sin(np.pi * (np.random.rand(1 * W, t) - 0.5))
        else:
            Xi = np.sin(np.pi * (np.random.rand(5 * W, t) - 0.5))
    else:
...
def evaluate_cubature_1d(W, Xi, f, K, basis, j, d):
    ...
    if basis == "Fourier_rand":
        ...
        phi = np.conjugate(np.cos(K @ np.arccos(np.conjugate(Xi.T)))) * (
            2.0 ** ((K != 0) / 2)
        )
        ...
        f_hat_temp = (scal * np.sqrt(2) / np.sqrt(W)) * dct(
            scal * f, type=1, norm="ortho"
        )
        idx = np.arange(0, max(K) + 1)
        f_hat = f_hat_temp[np.isin(idx, K)]
    elif basis in ["Fourier_SRLasso", "Fourier_SRLasso+"]:
        # Prepare indices
        K_unique, map_idx = unique_up_to_sign(K)
        K_half = K_unique * (-2) * np.pi
        # Start LASSO
        samples = torch.from_numpy(Xi).to(prec_type).to(device).requires_grad_(False)
        values = (
            torch.from_numpy(f)
            .to(prec_type)
            .view(-1, 1)
        ...
        indices = (
            torch.from_numpy(K_half).to(prec_type).to(device).requires_grad_(False)
        )
        f_hat_temp, _, _ = SR_Lasso_Fourier(samples, values, indices)
        f_hat_half = (f_hat_temp.detach().cpu().numpy()).squeeze()
        f_hat = expand_coefficients_up_to_sign(K, K_unique, map_idx, f_hat_half)
    elif basis in {"Fourier_OMP", "Fourier_OMP+"}:
        # Prepare indices
        K_unique, map_idx = unique_up_to_sign(K)
        K_half = K_unique * (-2) * np.pi
        # Start OMP
        ...
        f_hat_temp, _ = OMP_Fourier(samples, values, indices, num_iters=W, tol=1e-9)
        f_hat_half = (f_hat_temp.detach().cpu().numpy()).squeeze()
        f_hat = expand_coefficients_up_to_sign(K, K_unique, map_idx, f_hat_half)
    elif basis in ["Fourier_COSAMP", "Fourier_COSAMP+"]:
        # Prepare indices
        K_unique, map_idx = unique_up_to_sign(K)
        K_half = K_unique * (-2) * np.pi
        # Start COSAMP
        ...
        f_hat_temp, _ = COSAMP_Fourier(samples, values, indices, sparsity=W, tol=1e-5)
        f_hat_half = (f_hat_temp.detach().cpu().numpy()).squeeze()
        f_hat = expand_coefficients_up_to_sign(K, K_unique, map_idx, f_hat_half)
    elif basis in ["Cheby_SRLasso", "Cheby_SRLasso+"]:
        samples = torch.from_numpy(Xi).to(prec_type).to(device).requires_grad_(False)
        values = (
            torch.from_numpy(f)
            .to(prec_type)
        ...
        indices = torch.from_numpy(K).to(prec_type).to(device).requires_grad_(False)
        f_hat_temp, _, _ = SR_Lasso_Chebyshev(samples, values, indices)
        f_hat = (f_hat_temp.detach().cpu().numpy()).squeeze()
    elif basis in ["Cheby_OMP", "Cheby_OMP+"]:
        samples = torch.from_numpy(Xi).to(prec_type).to(device).requires_grad_(False)
        values = (
            torch.from_numpy(f)
            .to(prec_type)
        ...
        indices = torch.from_numpy(K).to(prec_type).to(device).requires_grad_(False)
        f_hat_temp, _ = OMP_Chebyshev(samples, values, indices, num_iters=W, tol=1e-9)
        f_hat = (f_hat_temp.detach().cpu().numpy()).squeeze()
    elif basis in ["Cheby_COSAMP", "Cheby_COSAMP+"]:
        samples = torch.from_numpy(Xi).to(prec_type).to(device).requires_grad_(False)
        values = (
            torch.from_numpy(f)
            .to(prec_type)
        ...
        indices = torch.from_numpy(K).to(prec_type).to(device).requires_grad_(False)
        f_hat_temp, _ = COSAMP_Chebyshev(samples, values, indices, sparsity=W, tol=1e-5)
        f_hat = (f_hat_temp.detach().cpu().numpy()).squeeze()
    else:
...
def evaluate_cubature(W, Xi, f, K, basis, dims, d):
    ...
    if basis == "Fourier_rand":
        ...
        # Build sparse matrix
        S = csr_matrix(
            (np.ones(len(W[2])), (W[2], np.arange(len(W[2])))), shape=(W[1], len(W[2]))
        )
        # LFFT
        A = LinearOperator(
            (len(f_full), len(f_hat)),
            matvec=lambda x: Ax_ssr1l(x, K, W, S),
            rmatvec=lambda b: Atb_ssr1l(b, K, W, S),
        )
        ...
        f_hat_temp, vals, vals_dual = SR_Lasso_Fourier(samples, values, indices)
        f_hat_half = (f_hat_temp.detach().cpu().numpy()).squeeze()
        f_hat = expand_coefficients_up_to_sign(K, K_unique, map_idx, f_hat_half)
    elif basis in ["Fourier_OMP", "Fourier_OMP+"]:
        # Prepare indices
        ...
        f_hat_temp, _ = OMP_Fourier(samples, values, indices, num_iters=W, tol=1e-7)
        f_hat_half = (f_hat_temp.detach().cpu().numpy()).squeeze()
        f_hat = expand_coefficients_up_to_sign(K, K_unique, map_idx, f_hat_half)
    elif basis in ["Fourier_COSAMP", "Fourier_COSAMP+"]:
        # Prepare indices
        ...
        for i in range(K.shape[0]):
            phi[i, :] = np.prod(
                np.cos(np.dot(np.ones((Xi.shape[0], 1)), K[i, :].reshape(1, -1)) * Xi),
                axis=1,
            ) * (2.0 ** (np.sum(K[i, :] != 0) / 2))
        f_hat, _, _, _ = lstsq(phi.T, f)
    elif basis == "Cheby_mr1l":
        ...
        A = LinearOperator(
            (len(f), len(f_hat)),
            matvec=lambda x: Ax_cmr1l(x, K, W),
            rmatvec=lambda b: Atb_cmr1l(b, K, W),
        )
        f_hat = lsqr(A, f)[0]
    elif basis == "Cheby_mr1l_subsampling":
        A = LinearOperator(
            (len(f), len(f_hat)),
            matvec=lambda x: Ax_cmr1lss(x, K, W),
            rmatvec=lambda b: Atb_cmr1lss(b, K, W),
        )
        f_hat = lsqr(A, f)[0]
    elif basis in ["Cheby_SRLasso", "Cheby_SRLasso+"]:
        samples = torch.from_numpy(Xi).to(prec_type).to(device).requires_grad_(False)
        ...
        indices = torch.from_numpy(K).to(prec_type).to(device).requires_grad_(False)
        f_hat_temp, vals, vals_dual = SR_Lasso_Chebyshev(samples, values, indices)
        f_hat = (f_hat_temp.detach().cpu().numpy()).squeeze()
    elif basis in ["Cheby_OMP", "Cheby_OMP+"]:
        samples = torch.from_numpy(Xi).to(prec_type).to(device).requires_grad_(False)
        values = (
        ...
        indices = torch.from_numpy(K).to(prec_type).to(device).requires_grad_(False)
        f_hat_temp, _ = OMP_Chebyshev(samples, values, indices, num_iters=W, tol=1e-7)
        f_hat = (f_hat_temp.detach().cpu().numpy()).squeeze()
    elif basis in ["Cheby_COSAMP", "Cheby_COSAMP+"]:
        samples = torch.from_numpy(Xi).to(prec_type).to(device).requires_grad_(False)
        values = (
...
def unique_up_to_sign(K):
    ...
    def canonical_row(row):
        ...
        if len(nz) == 0:
            return row.copy()
        return row if row[nz[0]] > 0 else -row

    row_to_new_idx = {}
    unique_rows = []
    index_map = np.empty(K.shape[0], dtype=int)

    for j, row in enumerate(K):
        canon = tuple(canonical_row(row))
        if canon not in row_to_new_idx:
            row_to_new_idx[canon] = len(unique_rows)
            unique_rows.append(np.array(canon, dtype=K.dtype))
        index_map[j] = row_to_new_idx[canon]

    K_unique = np.vstack(unique_rows)
    return K_unique, index_map


def expand_coefficients_up_to_sign(K, K_unique, index_map, c_unique):
    c_full = np.empty(len(K), dtype=np.complex128)
    for j, i in enumerate(index_map):
        if np.all(K[j] == 0):
            c_full[j] = c_unique[i]
        elif np.array_equal(K[j], K_unique[i]):
            c_full[j] = 0.5 * c_unique[i]
        else:
            c_full[j] = 0.5 * np.conjugate(c_unique[i])
    return c_full
