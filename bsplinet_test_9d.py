...
def bsplinet_test_9d_chat(freq_out):
    ...
    fhat[ind] = (
        fhat[ind] + bspline4t_chat_nd(freq_out[ind, :][:, [1, 4, 5, 7, 8]]).squeeze()
    )

    _, bspline2t_normcheb_d4_sq = bspline2t_normcheb(4)
    _, bspline4t_normcheb_d5_sq = bspline4t_normcheb(5)
    norm_fct_square = (
        bspline2t_normcheb_d4_sq
        + bspline4t_normcheb_d5_sq
        + 2 * bspline2t_chat_nd(np.zeros((1, 4))) * bspline4t_chat_nd(np.zeros((1, 5)))
    )
...
def bspline2t_chat_norm1(d=1):
    val = (
        np.sum(np.abs(bspline2t_chat_1d(np.arange(3))))
        + 3 / 128 * (4 - 5 * np.sqrt(3) / np.pi)
    ) ** d
    return val
...
def bspline4t_1d(x):
    ...
    x_cur = x[iremain[ind_pos]]
    val[iremain[ind_pos]] = (
        155 / 1536 - (5 / 32) * x_cur + (5 / 64) * x_cur ** 2 - (1 / 96) * x_cur ** 4
    )
    iremain = iremain[x[iremain] >= 1 / 2]
