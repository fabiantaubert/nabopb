...
gamma = {"type": "full", "N": 16, "sgn": "default"}

diminc = {"type": "default", "workers": 0}
...
if "F_ssr1l_result" in locals():
    ...
    if np.array_equal(F_ssr1l_result["index"], freq):
        ...
        print(
            f'Detection successful, samples: {np.sum(F_ssr1l_result["sample_sizes"])}'
        )
...
gamma = {"type": "full", "N": 16, "sgn": "default", "superpos": 4}
...
F_mr1l_approx_result = NABOPB(
    lambda x: bspline_test_10d(x), d, basis_flag3, gamma, diminc, s
)
...
if "F_r1l_approx_result" in locals():
    ...
    print(
        f'samples: {sum(F_r1l_approx_result["sample_sizes"])}, candidates: {sum(F_r1l_approx_result["cand_sizes"])}, time: {F_r1l_approx_result["run_times"][3]:.6f}'
    )
...
if "F_ssr1l_approx_result" in locals():
    ...
    print(
        f'samples: {sum(F_ssr1l_approx_result["sample_sizes"])}, candidates: {sum(F_ssr1l_approx_result["cand_sizes"])}, time: {F_ssr1l_approx_result["run_times"][3]:.6f}'
    )
...
if "F_mr1l_approx_result" in locals():
    ...
    print(
        f'samples: {sum(F_mr1l_approx_result["sample_sizes"])}, candidates: {sum(F_mr1l_approx_result["cand_sizes"])}, time: {F_mr1l_approx_result["run_times"][3]:.6f}'
    )
...
gamma = {"type": "full", "N": 16, "sgn": "non-negative"}
...
def f_cheby_test(x, freq, coef):
    ...
    for i in range(freq.shape[0]):
        mat[i, :] = np.prod(
            np.cos((np.ones((x.shape[0], 1)) * freq[i, :]) * x), axis=1
        ) * (2.0 ** (np.sum(freq[i, :] != 0) / 2))
...
if "C_ssmr1l_result" in locals():
    ...
    if np.array_equal(C_ssmr1l_result["index"], freq):
        ...
        print(
            f'Detection successful, samples: {np.sum(C_ssmr1l_result["sample_sizes"])}'
        )
        print(f"l_2-error: {err1_l2:e}")
        print(f"max-error: {err1_inf:e}")
    else:
...
gamma = {"type": "full", "w": 1, "N": 16, "sgn": "non-negative", "superpos": 5}

diminc = {"type": "default", "workers": 4}  # only for dyadic
...
if "C_rand_approx_result" in locals():
    print("====Variant 1====")
    val1 = C_rand_approx_result["val"] * np.sqrt(
        np.prod(2 ** (C_rand_approx_result["index"] != 0), axis=1)
    )
    ...
    print(f'max-error: {err1["inf"]:.6e}')
    print(
        f'samples: {sum(C_rand_approx_result["sample_sizes"])}, candidates: {sum(C_rand_approx_result["cand_sizes"])}, time: {C_rand_approx_result["run_times"][3]:.6f}'
    )

if "C_mr1l_approx_result" in locals():
    print("====Variant 2====")
    val2 = C_mr1l_approx_result["val"] * np.sqrt(
        np.prod(2 ** (C_mr1l_approx_result["index"] != 0), axis=1)
    )
    ...
    print(f'max-error: {err2["inf"]:.6e}')
    print(
        f'samples: {sum(C_mr1l_approx_result["sample_sizes"])}, candidates: {sum(C_mr1l_approx_result["cand_sizes"])}, time: {C_mr1l_approx_result["run_times"][3]:.6f}'
    )

if "C_ssmr1l_approx_result" in locals():
    print("====Variant 3====")
    val3 = C_ssmr1l_approx_result["val"] * np.sqrt(
        np.prod(2 ** (C_ssmr1l_approx_result["index"] != 0), axis=1)
    )
    ...
    print(
        f'samples: {sum(C_ssmr1l_approx_result["sample_sizes"])}, candidates: {sum(C_ssmr1l_approx_result["cand_sizes"])}, time: {C_ssmr1l_approx_result["run_times"][3]:.6f}'
    )
