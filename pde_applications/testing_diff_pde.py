...
def single_error(args):
    ...
    approx_sol = eval_cheby(
        np.hstack([points * 2 - 1, y_temp]), results["val"], results["index"]
    )
...
file_path = "results_diff_pde/s1000n64OMP+.pickle"
with open(file_path, "rb") as file:
    results = pickle.load(file)

# Run error computation
# err = parallel_error_random(10 ** 4, results)
y_test = np.load("diff_pde_testset_10000.npy")
err = parallel_error(y_test, results)
...
plt.title(
    "Boxplot of relative $L^2$ Errors for the parametric diffusion equation",
    fontsize=13,
)
