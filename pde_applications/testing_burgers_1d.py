...
def solve_burgers(a_coeffs, nu=0.05, alpha=2.0, nx=100):
    ...
    sol = solve_ivp(
        burgers_rhs, [0, 1.0], u0, t_eval=[1.0], method="Radau", rtol=1e-6, atol=1e-8
    )
...
def single_error(args):
    ...
    approx_sol = eval_cheby(
        np.hstack([points * 2 - 1, a_temp]), results["val"], results["index"]
    )

    # Compute relative L2-error (or absolute without /...)
...
file_path = "results_burgers_1d/s1000n64OMP+.pickle"
...
plt.title(
    "Absolute Error between exact solution and approximation at $t=1$", fontsize=14
)
plt.legend(fontsize=11)
plt.grid(True, linestyle="--", alpha=0.6)
plt.tight_layout()
