...
def solve_pois1d(var):
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        temp = (
            ...
            / (4 * np.pi**2 * ind**2)
            * (
                np.exp(2 * np.pi * 1j * np.expand_dims(ind, 0) * np.expand_dims(x, 1))
                - 1
            )
        )
...
for j in range(1, len(Ms)):
    Xi[zaehler : zaehler + (Ms[j] - 1) // 2, :] = np.cos(
        2
        * np.pi
        * np.mod(np.arange(1, (Ms[j] - 1) // 2 + 1).reshape(-1, 1) * zs[j, :], Ms[j])
        / Ms[j]
    )
    zaehler += (Ms[j] - 1) // 2
time_2 = time.time() - t2
...
A = LinearOperator(
    (len(f), len(f_hat)),
    matvec=lambda x: Ax_cmr1l(x, idx_set, W),
    rmatvec=lambda b: Atb_cmr1l(b, idx_set, W),
)
maxit = 100
tol = 1e-8
...
print(
    f"Total time: {time_1 + time_2 + time_3 + time_4 + time_5 + time_6} seconds. Times in detail:"
)
