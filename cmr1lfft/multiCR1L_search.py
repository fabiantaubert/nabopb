...
def multiCR1L_search(index_set_I, algno, MI=None, MI_index_I=None):
    ...
    if algno == 1:
        ...
        zs, Ms, reco_infos = multiCR1L_simple_Idec(index_set_I, None, MI, MI_index_I)
    ...
    for j in range(len(Ms)):
        reco_infos["kz_mod_M"].append(
            np.mod(np.dot(reco_infos["M_I"], zs[j, :]), Ms[j]).astype(int)
        )
...
def multiCR1L_simple_greedy(I, MI, MI_index_I):
    ...
    for j in range(zs.shape[0]):
        ...
        IMarker_mirror[MI_index_I[ind5] - 1, j] = 1
...
def multiCR1L_simple_Idec(I, L=None, MI=None, MI_index_I=None):
    reco_infos = {}
    ...
    N = np.max(I)
    M = int(nextprime(max(2 * (MI.shape[0] - 1), 2 * N) - 0.5))
    ...
    if I_rem.shape[0] > 0:
        zstmp, Mstmp, _ = multiCR1L_simple_Idec(I_rem, L)
    
        Msret = np.concatenate((
            np.array([M], dtype=int),
            np.asarray(Mstmp, dtype=int).reshape(-1)
        ))
        zsret = np.vstack((
            zs[ind[0], :],
            np.asarray(zstmp, dtype=int)
        ))
    else:
        Msret = np.array([M], dtype=int)
        zsret = zs[ind[0], :].reshape(1, -1)
...
def multiCR1L_improved_Idec(I, L, MI=None, MI_index_I=None):
    ...
    c = 2
    N = np.max(I)
    Mtmp = int(nextprime(max(2 * (MI.shape[0] - 1), max(2 * N + 1, 3)) - 0.5))
    ...
    while isinstance(p, np.ndarray) and len(p) > 1 and zaehler < 10:
        if startflag == 0:
                midpoint = int(np.ceil(len(p) / 2)) - 1
                Mtmp = int(p[midpoint])
        else:
            zaehler += 1
        ...
        if max_nImirror_tmp >= I.shape[0] / 2:
            I_remaining_inds = np.where(
                IMarker_mirror_tmp[:, ind] == False
            )[0]
            z = zs1[ind, :]
            M = int(Mtmp)
            ...
            if max_nImirror_tmp == 0:
                raise RuntimeError(
                    "No candidate lattice reconstructs any "
                    "remaining index."
                )
    ...
    if startflag == 1:
        print(
            "Warning: not each of the lattices reconstructs at least half of remaining cheb_freqs"
        )
...
def multiCR1L_simple_greedy_waawM(I):
    ...
    log_delta = -np.log(I.shape[0])
    N = np.max(I)
    M = int(nextprime(max(c * (MI.shape[0] - 1), 2 * N) - 0.5))
    L = int(np.ceil((c / (c - 1)) ** 2 * (np.log(I.shape[0]) - log_delta)/2))
    zs = np.random.randint(M, size=(L, MI.shape[1])) - 1
    IMarker_mirror1 = np.zeros((I.shape[0], zs.shape[0]), dtype=bool)
    labels = np.asarray(MI_index_I, dtype=np.intp)
    
    for j in range(zs.shape[0]):
        bins = np.mod(np.dot(MI, zs[j, :]), M).astype(np.intp)

        max_label = np.zeros(M, dtype=np.intp)
        min_label = np.full(M, I.shape[0] + 1, dtype=np.intp)
    
        np.maximum.at(max_label, bins, labels)
        np.minimum.at(min_label, bins, labels)
    
        occupied = max_label != 0
        admissible = occupied & (max_label == min_label)
        IMarker_mirror1[max_label[admissible] - 1, j] = True
...
def multiCR1L_simple_Idec_waawM(I, L=None):
    MI, MI_index_I = build_mirrored_index_set(I)
    reco_infos = {"M_I_index_I": MI_index_I, "M_I": MI}
    ...
    if I_rem.shape[0] > 0:
        zstmp, Mstmp, _ = multiCR1L_simple_Idec_waawM(
            I_rem, L
        )
...
def multiCR1L_improved_Idec_waawM(I, L=None):
    MI, MI_index_I = build_mirrored_index_set(I)
    c = 2
    reco_infos = {"M_I_index_I": MI_index_I, "M_I": MI}
    ...
    while len(p) > 1 and zaehler < 10:
        ...
        for j in range(zs1.shape[0]):
            bins = np.mod(
                np.dot(MI, zs1[j, :]), Mtmp
            ).astype(np.intp)
        
            max_label = np.zeros(Mtmp, dtype=np.intp)
            min_label = np.full(
                Mtmp, I.shape[0] + 1, dtype=np.intp
            )
    ...
    if I_rem.shape[0] > 0:
        zstmp, Mstmp, _ = multiCR1L_improved_Idec_waawM(
            I_rem, L
        )
        ...
        Msret = np.array([M], dtype=int)
        zsret = np.asarray(z, dtype=int).reshape(1, -1)
