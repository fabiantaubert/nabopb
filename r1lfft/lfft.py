...
def lhcfft_notransp(H, z, M, f_hat):
    ...
    k = k.astype(np.int64)
    fhat1 = np.zeros(M, dtype=np.complex128)
    fhat1[: np.max(k)] = np.bincount(k - 1, weights=np.real(f_hat)) + 1j * np.bincount(
        k - 1, weights=np.imag(f_hat)
    )  # Accumulate values at indices k-1
