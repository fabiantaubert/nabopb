...
def bspline_test_10d_fouriercoeff(freq_out):
    ...
    norm_fct_square = (
        3
        + 2 * bspline_o2_hat(np.zeros((1, 3))) * bspline_o4_hat(np.zeros((1, 4)))
        + 2 * bspline_o2_hat(np.zeros((1, 3))) * bspline_o6_hat(np.zeros((1, 3)))
        + 2 * bspline_o4_hat(np.zeros((1, 4))) * bspline_o6_hat(np.zeros((1, 3)))
    )
...
def bspline_o2_hat(k):
    ...
    for t in range(k.shape[1]):
        ...
        if ind.size > 0:
            val[ind] = (
                val[ind]
                * (bspline_sinc(np.pi / 2 * k[ind, t]) ** 2)
                * np.cos(np.pi * k[ind, t])
            )
        val = np.sqrt(3 / 4) * val
    return val
...
def bspline_o4_hat(k):
    ...
    for t in range(k.shape[1]):
        ...
        if ind.size > 0:
            val[ind] = (
                val[ind]
                * (bspline_sinc(np.pi / 4 * k[ind, t]) ** 4)
                * np.cos(np.pi * k[ind, t])
            )
        val = np.sqrt(315 / 604) * val
    return val
...
def bspline_o6_hat(k):
    ...
    for t in range(k.shape[1]):
        ...
        if ind.size > 0:
            val[ind] = (
                val[ind]
                * (bspline_sinc(np.pi / 6 * k[ind, t]) ** 6)
                * np.cos(np.pi * k[ind, t])
            )
        val = np.sqrt(277200 / 655177) * val
    return val
